...
ExportDSERsFlag  = false;           % Flag to determine if BRIRs should be exported with direct sound and early reflections separated (WAV-files only).
...
parfor iDir = 1 : nDirs
    ...
    if strcmp(BRIR_data.RTModMode, 'Txx')
        BRIR_early(:, :, iDir) = Modify_Reverb_Slope(BRIR_data, BRIR_early_temp);
    else
        BRIR_early(:, :, iDir) = BRIR_early_temp;
    end
end
close(hbar);
clear iDir hbar;
...
lenBeforeRemoval = size(BRIR_early, 1);
[BRIR_early, BRIR_late] = Remove_BRIR_Delay(BRIR_early, BRIR_late, -20);
samplesRemoved = lenBeforeRemoval - size(BRIR_early, 1) + 1;

% also cut the pressure RIR to the same size
P = SRIR_data.P_RIR(samplesRemoved:size(BRIR_late)+samplesRemoved-1);
...
% -----------------------------------------------------------------------
% 8. Modify slope by edc correction, if specified

if strcmp(BRIR_data.RTModMode, 'EDC')
    [BRIR_early, BRIR_late] = ...
        Modify_Reverb_Slope_EDC(BRIR_data, P, BRIR_early, BRIR_late, Plot_data);
end

% -----------------------------------------------------------------------
% 9. Split the BRIRs into components by time windowing
...
% -----------------------------------------------------------------------
% 10. Normalize
[BRIR_DSER, BRIR_LR, BRIR_DS, BRIR_ER] = ...
    Normalize(BRIR_data, BRIR_DSER, BRIR_LR, BRIR_DS, BRIR_ER);
